...
const Step3Report = ({ report }) => {
  ...
  if (finalScore >= 8) {
    ...
    shortTagline = "Excellent clasrity and sturucted responses.";
  }
...
// wrap text so it fits inside box
const wrappedAdvice = doc.splitTextToSize(advice, contentWidth - 20);

doc.text(wrappedAdvice, margin + 10, currentY + 18);
    currentY += 50;
    autoTable(doc, {
      startY: currentY,
  ...
  <div className='md:mb-10 w-full flex items-start gap-4 flex-wrap'>
          ...
          <div> <h1 className='text-3xl flex-nowrap font-bold text-gray-800'>Interview Analytics Dashboard</h1>
            <p className='text-gray-500 mt-2'>AI-powered performance insights</p>
          </div>
      </div>
        <button
          onClick={downloadPDF}
          className='bg-emerald-600 hover:bg-emerald-700 text-white py-3 rounded-xl shadow-md transition-all duration-300 font-semibold text-sm sm:text-base text-nowrap'>Download PDF</button>
      </div>
      <div className='grid grid-cols-1 lg:grid-cols-3 lg:gap-8'>
        <div className='space-y-6'>
          ...
          <motion.div initial={{ opacity: 0 }}
            ...
            <h3 className='text-base sm:text-lg font-semibold text-gray-700 mb-6'>Skill Evalutation</h3>
        ...
        <div className='lg:col-span-2 space-y-6'>
          ...
          <motion.div initial={{ opacity: 0 }}
            ...
            <div className='space-y-6'>{questionWiseScore.map((q, i) => (
              <div key={i} className='bg-gray-50 p-4 sm:p-6 rounded-xl sm:rounded-2xl border-gray-200'>
                <div className='flex flex-col sm:flex-row sm:justify-between sm:items-start gap-3 mb-4'>
                  ...
                  <div className='bg-green-100 text-green-600 px-3 py-1 rounded-full font-bold text-xs sm:text-sm w-fit'>{q.score ?? 0}/10</div>
                </div>
